...
export default function CourseOutline({
  ...
  useEffect(() => {
    ...
    if (roadmap?.phases?.length && expandedPhase === null) {
      for (const phase of roadmap.phases) {
        if (phase.lessons.some((lesson) => !completedLessons.includes(lesson._id))) {
  ...
  const handleButtonClick = () => {
    if (activeView === "outline") {
      ...
      if (nextLessonId) onSelectLesson(nextLessonId)
    } else {
      setShowSidebar(true)
    }
  }
      ...
      <div className="p-5 border-b">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm font-medium">{courseProgress.percentage}%</div>
          </div>

          <Progress value={courseProgress.percentage} className="h-2" />

          <Button className="w-full mt-2" size="sm" variant={"outline"} onClick={handleButtonClick}>
        ...
        {allPhases.length === 0 ? (
          ...
          <div className="p-5 space-y-5">
            {allPhases.map((phase) => {
              ...
              const phasePercentage = lessons.length > 0 ? Math.round((phaseCompletedCount / lessons.length) * 100) : 0
              const isPhaseComplete = phasePercentage === 100

              return (
                <div key={phase._id} className="border rounded-lg overflow-hidden">
                  {/* Phase Header */}
                  <button
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        isPhaseComplete ? "bg-primary" : "border-2 border-muted bg-white",
                      )}
                    >
                      {isPhaseComplete && <Check className="h-3.5 w-3.5 text-primary-foreground" />}
                    </div>

                    <div className="flex-1 text-left">
                      <div className="font-medium text-sm">{phase.phaseTitle}</div>
                      <div className="text-xs text-muted-foreground mt-0.5">
                        {phaseCompletedCount} of {lessons.length} lessons completed
                      </div>
                    </div>
                          ...
                          >
                            <div
                              className={cn(
                                ...
                                isCompleted ? "bg-primary" : "border-2 border-muted bg-white",
                              )}
                            >
