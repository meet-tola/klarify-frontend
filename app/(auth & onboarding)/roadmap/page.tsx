...
import { selectedSearchSkill, getRoadmap, clearUserSkills, generateRoadmapSectionContent } from "@/lib/api"
...
export default function RoadmapPage() {
  ...
  if (isLoading || isFetchingContent) {
    return <LoadingScreen message={isFetchingContent ? "Fetching Content..." : "Loading..."} />
  }
        ...
        {selectedSkill && skillDetails ? (
          <>
            <SkillDetailsCard
              ...
              onGenerateRoadmap={
                user?.user.learningPath && user?.user.learningPath.length > 0 ? handleGetRoadmap : handleGenerateRoadmap
              }
              hasExistingRoadmap={!!(user?.user.learningPath && user?.user.learningPath.length > 0)}
            />

            {showRoadmap && <RoadmapPhases phases={phases} onStartLearning={handleStartLearning} />}
          </>
        ) : (
          <>
            <FindCareerPath
